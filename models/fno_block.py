...
class FactorizedSpectralConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_modes, n_layers=1, scale='auto', separable=False,
        ...
        for i in range(num_weights):
            ...
            # Choose which weights to use based on the joint_factorization setting.
            if self.joint_factorization:
                # True means all layers use a single shared weight tensor.
                current_weight = self.weight
            else:
                # False means use the weight corresponding to each layer in the ModuleList.
